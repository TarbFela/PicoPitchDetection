...
uint32_t midi_control(int ctrl_message) {
  ...
  if(ctrl_message == CTRL_NO_NOTE) { // no note
  ...
  if( BOTTOM_MIDI_NOTE <= ctrl_message && ctrl_message <= TOP_MIDI_NOTE) {
    if( ctrl_message != current_on_note) {//if we're already playing this note, don't do anything
...
int midi_note_on(uint8_t pitch) {
  ...
  if( BOTTOM_MIDI_NOTE > pitch || pitch > TOP_MIDI_NOTE) return -1;
